...
(function() {
  var CND, LTSORT, badge, debug, log, rpr, step;
  ...
  this.new_cache = function(stamper) {
    var R, arity, type;
    if (stamper == null) {
      stamper = null;
    }
    if (stamper == null) {
      stamper = this.HELPERS.file_stamper;
    }
    if ((type = CND.type_of(stamper)) !== 'function') {
      throw new Error("expected a function, got a " + type);
    }
    if ((arity = stamper.length) !== 2) {
  ...
  this.HELPERS.shell = function(command, handler) {
