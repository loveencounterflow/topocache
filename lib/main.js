...
(function() {
  ...
  this._find_faults = function(me, first_only, handler) {
    step((function(_this) {
      return function*(resume) {
        var R, boxed_chart, cmp_name, cmp_trending_idx, i, indexed_chart, indexed_trend, len, messages, nfo, ref, ref_chart_idx, ref_name, ref_trend_idx, warn_missing;
        ...
        for (ref_name in indexed_chart) {
          ...
          ref = me['graph']['precedents'].get(ref_name);
          for (i = 0, len = ref.length; i < len; i++) {
            cmp_name = ref[i];
            ...
            nfo = _this.get_fix(me, cmp_name, ref_name, null);
            if (nfo == null) {
              nfo = {
                cause: comparison,
                effect: reference,
