...
(function() {
  var CND, LTSORT, PATH, badge, debug, help, log, rpr, step, warn;
  ...
  this.apply_fixes = function(me, handler) {
    step((function(_this) {
      return function*(resume) {
        var Z, command, dt, error, fault, fix, kind, output, ref, run, runs, t0, t1, type;
        runs = [];
        Z = {
          runs: runs,
          t0: new Date()
        ...
        while ((fault = (yield _this.find_first_fault(me, resume))) != null) {
          ...
          t0 = new Date();
          fix = fault.fix;
          if ((type = CND.type_of(fix)) !== 'list') {
            return handler(new Error("expected a list, got a " + type));
          }
          kind = fix[0], command = fix[1];
          switch (kind) {
            case 'shell':
              ref = (yield _this.HELPERS.shell(me, command, resume)), output = ref.stdout, error = ref.stderr;
              break;
            default:
