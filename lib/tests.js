...
(function() {
  var CND, FS, LTSORT, PATH, TC, badge, debug, demo_crc32, echo, help, include, info, log, rpr, step, templates_home, test, test_data_home, test_timer_resolution, urge, warn, whisper,
  ...
  if (module.parent == null) {
    include = ["create cache object", "register file objects", "register file objects with complex keys", "find fault(s) (1)", "find fault(s) (non-existent file)", "find single fault", "find multiple faults", "align multiple faults (1)", "align multiple faults (2)", "fixes can be strings, lists", "catalog"];
    this._prune();
    demo_crc32 = function(handler) {
      return step((function(_this) {
        return function*(resume) {
          var checksum, home, i, len, path, paths;
          paths = ['shape/shape-breakdown-formula.txt', 'shape/shape-breakdown-formula-consolidated.txt', 'shape/shape-breakdown-formula-corrections.txt', 'shape/shape-breakdown-formula-naive.txt', 'shape/shape-factor-hierarchy.txt', 'shape/shape-figural-themes.txt', 'shape/shape-guides-similarity.txt', 'shape/shape-similarity-identity.txt', 'shape/shape-strokeorder-zhaziwubifa.txt'];
          home = PATH.resolve(__dirname, '../../jizura-datasources/data/flat-files');
          for (i = 0, len = paths.length; i < len; i++) {
            path = paths[i];
            path = PATH.resolve(home, path);
            checksum = (yield TC.FILEWATCHER.checksum_from_path(null, path, resume));
            help(CND.orange(checksum), CND.grey(path));
          }
          return handler();
        };
      })(this));
